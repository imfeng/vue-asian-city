...
export default {
  head() {
    return {
      title: '南韩娱乐网 - ' + this.title + ' - ' + this.interval + 's',
      meta: [
        { charset: 'utf-8' },
        ...
        { name: 'viewport', content: 'width=device-width, initial-scale=1' },
        {
          ...
          content:
            this.title +
            '祝您中奖，好处多多。开奖器采用美商Smartplay Inc生产之乱数产生器，每季都有专员检验查核，本司为业界最公正的第三方开奖平台！'
        },

        { hid: 'keywords', name: 'keywords', content: this.title + ',开奖网,摇彩网,南韩娱乐网' }
      ],
  ...
  methods: {
    ...
    updateApi: function() {
      this.$axios.$get('/vv16888/api.php?type=' + this.gameId).then(res => {
        if (res instanceof Object) {
          ...
          if (data instanceof Array) {
            ...
            if (
              this.historiesList !== [] &&
              this.historiesList[0] == 1 &&
              this.historiesList[0].expect != this.item.expect
            )
    ...
    getHistories: function(plus = true) {
      ...
      this.$axios.$get('/vv16888/api.php?type=' + this.gameId + '&rows=' + this.historyCnt).then(res => {
        if (res instanceof Object) {
          let data = res['data']
          if (data instanceof Array) {
            this.historiesList = data.map(v => {
              v.codes = v.opencode.split(',')
              return v
            })
          }
        }
